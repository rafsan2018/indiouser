   ...
   useEffect(() => {
    ...
    return (
            ...
            <div className="tw-flex xs:tw-flex-col xl:tw-flex-row tw-mt-10 lg:tw-space-x-20 xs:tw-space-x-0">
                ...
                <div className="tw-flex tw-flex-col tw-rounded-lg tw-border-2 tw-border-green-500 tw-shadow-md tw-mx-5">
                            ...
                            <div>
                                <div className='tw-flex xl:tw-flex-row xs:tw-flex-col p-5 xl:tw-space-x-10 xs:tw-space-x-0 xs:tw-space-y-2 xl:tw-space-y-0' key={items.id}>
                                    <div className="tw-flex xl:tw-flex-col xs:tw-flex-row tw-space-x-2">
                                        <h2 className="tw-text-sm tw-text-green-500 tw-font-semibold">Product Photo</h2>
                                        <hr></hr>
                                    ...
                                    <div>
                                        <h2 className="tw-text-sm tw-text-green-500 tw-font-semibold">Product Details</h2>
                                        <hr></hr>
                                        <div className="tw-space-y-5 tw-pt-2">
                                            <h1 className="tw-text-xs tw-text-green-500 tw-font-semibold tw-uppercase ">Name: {items.title}</h1>
                                            <h1 className="tw-text-xs  tw-text-green-500 tw-font-semibold tw-uppercase">Color: {items.color}</h1>
                                            <h1 className="tw-text-xs  tw-text-green-500 tw-font-semibold tw-uppercase">Size: {items.size}</h1>
                                        </div>
                                    </div>
                                    <div className="tw-flex xl:tw-flex-col xs:tw-flex-row tw-text-green-500 tw-uppercase">
                                        <h2 className="tw-text-xs tw-text-green-500 tw-font-semibold tw-pt-1">Quantity</h2>
                                        <hr></hr>
                                        <div className="tw-flex tw-flex-row xl:tw-mt-4 xs:tw-mt-0 ">
                                            <span className="tw-border-2 tw-border-green-500 tw-px-4 tw-text-xs tw-font-semibold">{items.qty}</span>
                                        </div>
                                    </div>
                                    <div className="tw-flex xl:tw-flex-col xs:tw-flex-row tw-space-x-2 tw-text-green-500 tw-font-semibold tw-uppercase tw-text-sm">
                                        <h2 className="tw-text-xs tw-text-green-500 tw-font-semibold tw-pt-1">Price</h2>
                                        <hr></hr>
                                    ...
                                    <div className="tw-flex xl:tw-flex-col xs:tw-flex-row tw-space-x-2 tw-text-green-500 tw-font-semibold tw-uppercase tw-text-sm">
                                        <h2 className="tw-text-xs tw-text-green-500 tw-font-semibold tw-pt-1">Total</h2>
                                        <hr></hr>
                                        <p className="xl:tw-mt-4 xs:tw--mt-0.5">
                    ...
                    <div className="tw-rounded-lg tw-border-2 tw-border-green-500 tw-shadow-md hover:tw-opacity-75 tw-px-5 tw-space-y-5 tw-mx-5">
                        <h1 className="tw-text-center tw-text-xl tw-text-green-500 tw-font-bold">Shipping Information</h1>
                        <h1 className="tw-text-sm tw-font-bold">Name: {data && data.order.shipping.name}</h1>
                        <h2 className="tw-text-sm tw-font-bold"> Phone: {data && data.order.shipping.phone}</h2>
                        <h2 className="tw-text-sm tw-font-bold">Addrees: {data && data.order.shipping.address}</h2>
                        <h2 className="tw-text-sm tw-font-bold">City: {data && data.order.shipping.city}</h2>
                        <h2 className="tw-text-sm tw-font-bold">State: {data && data.order.shipping.state}</h2>
                    </div>
                </div>
        ...
        <form method="post" action="http://indigo.my/payment/billplzpost.php" className="tw-flex tw-flex-col tw-pb-4">
            ...
            <input type="hidden" name="reference_2_label" value={`Quantity:${data && (data.order.orderDetails.reduce((a, b) => a + ((b['qty']) || 0), 0)) }`}/>
            <input type="hidden" name="reference_2" value="7"/>
            <input type="hidden" name="description" value ={`Products:${data && (data.order.orderDetails.reduce((a, b) => a + ((b['title,']) || 0), 0)) }`}/>
